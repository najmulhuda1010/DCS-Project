...
class LoanController extends Controller
{
    ...
    public function action_btn(Request $request)
    {
        ...
        $document_url = "http://scmtest.brac.net/dcs/DocumentManager?doc_id=$doc_id&projectcode=$projectcode&doc_type=loan&pin=$pin&role=$role&branchcode=$branchcode&action=$action&comment=$comment";
        // dd($document_url);
    ...
    public function approve_loan(Request $request)
    {
        ...
        $document_url = "http://scmtest.brac.net/dcs/DocumentManager?doc_id=$doc_id&projectcode=$projectcode&doc_type=loan&pin=$pin&role=$role&branchcode=$branchcode&action=$action";
        //dd($document_url);
        Log::channel('daily')->info('Document_url : ' . $document_url);
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $document_url);
    ...
    public function closeLoan(Request $request)
    {
        ...
        $url = "http://scmtest.brac.net/dcs/LastOneCloseLoanBehavior?token=7f30f4491cb4435984616d1913e88389&key=5d0a4a85-df7a-scapi-bits-93eb-145f6a9902ae&BranchCode=$branchcode&MemberId=$erp_mem_id";
        $response = Http::get($url);
    ...
    public function Savings_Behaviours(Request $request)
    {
        ...
        $url = "http://scmtest.brac.net/dcs/operation/savingsTransaction?" . $urlparams;
        echo $url;
        $ch = curl_init();
