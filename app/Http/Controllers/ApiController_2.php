...
class ApiController extends Controller
{
  ...
  public function getAdmissions($branchcode, $projectcode, $pin)
  {
    ...
    if ($admissiondata->isEmpty()) {
      ...
      foreach ($admissiondata as $data) {
        ...
        $dochistorys = $dochistory->comment ?? null;
  ...
  public function Index(Request $req) //no need
  {
    $baseUrl = url('');
  ...
  public function erpVOList(Request $req) //no need
  {
    $dberp = $this->dberp;
    $db = $this->db;
    $token = Request::input('token');
  ...
  public function ImageUpload(Request $req)
  {
    ...
    $baseurl = 'http://34.81.47.25/uploads/';
    ...
    if ($token == '7f30f4491cb4435984616d1913e88389') {
      ...
      if ($loandata->isEmpty()) {
        ...
        foreach ($loandata as $data) {
          ...
          $rca = DB::table($db . '.rca')->where('loan_id', $data->id)->first();
          Log::info("Primary Earner" . $rca->primary_earner . "-" . $data->id);
          $PrimaryEarner = DB::table($db . '.payload_data')->select('data_name')->where('data_type', 'primaryEarner')->where('data_id', $rca->primary_earner)->first();
          $PrimaryEarnerName = $PrimaryEarner->data_name;
          // dd($PrimaryEarner);
          Log::info("Primary Earner full set" . json_encode($PrimaryEarner));
          $bmPrimaryEarner = DB::table($db . '.payload_data')->select('data_name')->where('data_type', 'primaryEarner')->where('data_id', $rca->bm_primary_earner)->first();
          if ($bmPrimaryEarner) {
            $bmPrimaryEarnerIs = $bmPrimaryEarner->data_name;
          } else {
          ...
          $arrayData['rca'] = array(
            "id" => $rca->id,
            "loan_id" => $rca->loan_id,
            "primary_earner" => $PrimaryEarnerName,
  ...
  public function admissionDataProccessForErp($id)
  {
    ...
    $nominees = array();
    if ($data->NomineeNidType != '3' or $data->NomineeNidType != '4') {
      $NomineeIdExpiredate = null;
      $issuePlace = null;
    } else {
    ...
    if ($data->MainIdTypeId != '3' or $data->MainIdTypeId != '4') {
      $expiryDate = null;
      $issuePlace = null;
    } else {
      $expiryDate = $data->ExpiryDate;
    ...
    if ($data->SpouseCardType != '3' or $data->SpouseCardType != '4') {
      $SpouseIdExpiredate = null;
      $issuePlace = null;
    } else {
      //$expiryDate = $data->ExpiryDate;
    ...
    $arrayData[] = array(
      ...
      "idCard" => array(
        ...
        "idCardNo" => $data->IdNo,
        "issueDate" => null,
        "issuePlace" => $issuePlace, //$data->IssuingCountry,
      ),
      "maritalStatusId" => $data->MaritalStatusId,
      "memberId" => $data->MemberId,
      ...
      "spouseIdCard" => array(
        ...
        "issueDate" => null,
        "issuePlace" => $issuePlace, //$data->SpouseIdPlaceOfissue,
      ),
  ...
  public function GetErpPendingAdmissionDataStatus($branchcode, $projectcode, $applicationdate)
  {
    ...
    $curl = curl_init();
    $url2 = "https://bracapitesting.brac.net/dcs/v1/branches/$branchcode/buffer-members?projectCode=$projectcode&applicationDate=$applicationdate";
    curl_setopt_array($curl, array(
      //CURLOPT_URL => $url2 . 'branches/' . $branchcode . '/buffer-members?projectcode = ' . $projectcode . '&applicationDate = ' . $applicationdate,
      CURLOPT_URL => $url2,
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
  ...
  public function GetErpPendingLoanDataStatus($branchcode, $projectcode, $applicationdate)
  {
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => $url2 . 'branches/' . $branchcode . '/buffer-loan-proposals?projectcode=' . $projectcode . '&applicationDate=' . $applicationdate,
      CURLOPT_RETURNTRANSFER => true,
      CURLOPT_ENCODING => "",
      CURLOPT_MAXREDIRS => 10,
      CURLOPT_TIMEOUT => 30,
  ...
  public function Create_Insert(Request $request)
  {
    ...
    //$field_def = array("IsRefferal" => "", "RefferedById" => "", "pin" => "pin", "vo_code" => "orgno");
    $field_def = array(
      "vo_code" => "orgno", "branch_code" => "branchcode", "pin" => "pin", "project_code" => "projectcode", "enroll_id" => "entollmentid", "erp_mem_id" => "MemberId", "is_ref" => "IsRefferal", "refby" => "RefferedById", "refname" => "ReffererName",
      "refphone" => "ReffererPhone", "mem_category" => "MemberCateogryId", "applicant_name" => "ApplicantsName", "mainid_type" => "MainIdTypeId", "mainid_number" => "IdNo", "other_idtype" => "OtherIdTypeId", "other_idnumber" => "OtherIdNo", "expiredate" => "ExpiryDate", "place_ofissue" => "IssuingCountry", "dob" => "DOB", "mother_name" => "MotherName", "father_name" => "FatherName",
      "education" => "EducationId", "occupation" => "Occupation", "phone" => "Phone", "isbkash" => "IsBkash", "wallet_no" => "WalletNo", "wallet_owner" => "WalletOwner", "present_adds" => "PresentAddress",
      "present_upazila" => "presentUpazilaId", "is_same_addss" => "IsSameAddress", "permanent_adds" => "PermanentAddress", "permanent_upazila" => "parmanentUpazilaId", "matrial" => "MaritalStatusId",
      "spouse_name" => "SpouseName", "spouse_card_type" => "SpouseCardType", "spouse_nid" => "SpouseNidOrBid", "spouse_dob" => "SposeDOB", "spouse_occ" => "SpuseOccupationId", "spouse_id_expiredate" => "SpouseIdExpiredate", "spouse_id_place_ofissue" => "SpouseIdPlaceOfissue",
      "total_family_mem" => "FamilyMemberNo", "total_child" => "NoOfChildren", "nominee_name" => "NomineeName", "nominee_dob" => "NomineeDOB",
      "nominee_nid_type" => "NomineeNidType", "nominee_nid_no" => "NomineeNidNo", "nominee_phone_number" => "NomineePhoneNumber", "nominee_id_expiredate" => "NomineeIdExpiredate", "nominee_id_place_ofissue" => "NomineeIdPlaceOfissue",
      "relationship" => "RelationshipId", "primary_earner" => "PrimaryEarner", "applicant_single_pic" => "ApplicantSinglePic", "applicant_photo" => "ApplicantCpmbinedImg", "ref_photo" => "ReffererImg", "refid_photo" => "ReffererIdImg",
      "nidfront_photo" => "FrontSideOfIdImg", "nidback_photo" => "BackSideOfIdimg", "nominee_nid_photo" => "NomineeIdImg", "nominee_nid_front" => "NomineeNidFront", "nominee_nid_back" => "NomineeNidBack",
      "spouse_nid_photo" => "SpuseIdImg", "spouse_nid_front" => "SpouseNidFront", "spouse_nid_back" => "SpouseNidBack", "passbook_required" => "PassbookRequired", "genderid" => "GenderId",
      "savingsProductId" => "SavingsProductId", "targetAmount" => "TargetAmount", "permanentDistrictId" => "PermanentDistrictId", "presentDistrictId" => "PresentDistrictId", "surveyid" => "surveyid"
    );


    $admission_data = '{"token":"xxxxxxxxx","appid":"xxxxxxxxx","flag":1,"data":[{"vo_code":"2080","branch_code":"2867","pin":"00160727","project_code":"015","enroll_id":"123e4567-e89b-12d3-a457-426614174002","erp_mem_id":null,"is_ref":0,"refby":null,"refname":null,"refphone":null,"mem_category":1,"applicant_name":"Rina Sarkar","mainid_type":5,"mainid_number":"6666666666","other_idtype":null,"other_idnumber":null,"expiredate":null,"place_ofissue":"Bangladesh","dob":"1998-10-11","mother_name":"Mina Sarkar","father_name":"Haripoda Sarkar","education":5,"occupation":2,"phone":"01765999999","isbkash":1,"wallet_no":"01765999999","wallet_owner":1,"present_adds":"House\/Village:-Ashkona,Road NO\/Word No:-5,Union\/Pouroshova:-Dakkhinkhan","present_upazila":166,"is_same_addss":1,"permanent_adds":"House\/Village:-Ashkona,Road NO\/Word No:-5,Union\/Pouroshova:-Dakkhinkhan","permanent_upazila":166,"matrial":2,"spouse_name":"Harihar Sarkar","spouse_card_type":5,"spouse_nid":"8888888888","spouse_dob":"1984-10-11","spouse_occ":6,"spouse_id_expiredate":null,"spouse_id_place_ofissue":"Bangladesh","total_family_mem":"6","total_child":"3","nominee_name":"Harihar Sarkar","nominee_dob":"1984-10-11","nominee_nid_type":5,"nominee_nid_no":"8888888888","nominee_phone_number":"01877777777","nominee_id_expiredate":null,"nominee_id_place_ofissue":"Bangladesh","relationship":13,"primary_earner":1,"applicant_single_pic":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:52IMG_20221011_160748067.jpg","applicant_photo":"","ref_photo":null,"refid_photo":null,"nidfront_photo":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:53IMG_20221011_160804147.jpg","nidback_photo":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:53IMG_20221011_160815060.jpg","nominee_nid_photo":null,"nominee_nid_front":null,"nominee_nid_back":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:54IMG_20221011_160823383.jpg","spouse_nid_photo":null,"spouse_nid_front":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:54IMG_20221011_160838466.jpg","spouse_nid_back":"http:\/\/scmtest.brac.net\/uploads\/2867\/2022-10-11 10:08:55IMG_20221011_160847492.jpg","passbook_required":0,"genderid":2,"savingsProductId":11,"targetAmount":100,"permanentDistrictId":14,"presentDistrictId":14,"surveyid":null}],"extra":"[]"}';
    $data_set = json_decode($admission_data, true);
    $data = $data_set["data"];
    ...
    foreach ($data as $value) {
      ...
      foreach ($value as $k => $v) {
        ...
        /*$up = $this->starts_with_upper($f);
        if ($up) {
          $f = '"' . $f . '"';
        }*/
        // $f = '"' . $f . '"';
        // $fv = '"' . $v . '"';
        //array_push($arrayobject, $fv);
        //array_push($arrayvalues, $fv);
      }
      // Replace last , with ) for $fv
      //$fv = substr_replace($fv, '', strrpos($likes, ','), 1);
    }
    //echo $fv;
    $fvs = rtrim($fv, ","); // substr_replace($fv, '', strrpos($likes, ','), 1);
    //echo $fvs;
    $insertquerys = DB::Table('dcs.admissions')->insert([$fvs]);
    echo $insertquerys;
    // $ce =  implode(",", $arrayobject);
    // $d = array($ce);
    // $values = implode(",", $arrayvalues);
    //DB::table('dcs.admissions')->insert(['orgno' => "2080", 'branchcode' => "2867", 'pin' => "00160727", 'projectcode' => "015", 'entollmentid' => "3b06ccff-b1d3-430c-8713-b2443g0ff5e0"]);

    //$insertquery = DB::Table('dcs.admissions')->insert([$d]);
    //$insertquery = DB::select(DB::raw("Insert into dcs.admissions($ce) VALUES ($values)"));
    //echo $insertquery;
  }
  public function Table_Column(Request $request)
  {
    $row = DB::Table("dcs.admissions")->get();
    ...
    foreach ($row as $key => $value) {
      ...
      foreach ($value as $keys => $values) {
        $object = '"' . $keys . '"=>"",';
        echo $object;
        //array_push($arrys, $keys);
        //array_push($keyarrays, $object);
      }
      $i++;
    }
    $ce =  implode(",", $arrys);
    $value = implode(",", $keyarrays);
    $insertquery = "insert into dcs.admissions($ce) VALUES('$values')";
    echo $insertquery;
  }
  ...
  public function CheckSomeData(Request $re)
  {
    $orgno = '2080';
    $branchcode = '';
    $IsRefferal = TRUE;
    $entollmentid = '123e4567-e89b-12d3-a457-426614174001';
    $dt = '"orgno" => "2080", "branchcode" => "", "IsRefferal" => 1, "entollmentid" => "123e4567-e89b-12d3-a457-426614174005"';
    $db = DB::Table("dcs.admissions")->insert([$dt]);
    echo $db;
    // DB::select(DB::raw("INSERT INTO dcs.admissions(orgno,branchcode,("IsRefferal"),entollmentid) VALUES ('2083', '8320', 1, '123e4567-e89b-12d3-a457-426614174000')"));
  }
  ...
  public function SurveyDataSync()
  {
    $token = Request::input('token');
    $branchcode = Request::input('branchcode');
    $ProjectCode = Request::input('projectcode');
    ...
    $db = $this->db;
    if ($branchcode != null and $ProjectCode == null) {
      $surveydata = DB::table($db . '.surveys')->where('branchcode', $branchcode)->orderBy('id', 'desc')->get();
    } elseif ($branchcode != null and $ProjectCode != null) {
      $surveydata = DB::table($db . '.surveys')->where('branchcode', $branchcode)->where('projectcode', $ProjectCode)->orderBy('id', 'desc')->get();
    }
    if (empty($surveydata)) {
      return json_encode(array("code" => 200, "message" => "Data Not Found", "data" => []));
    } else {
      return json_encode(array("code" => 200, "message" => "", "data" => $surveydata));
    }
  }
}
