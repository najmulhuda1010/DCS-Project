...
class FormConfigController extends Controller
{
	...
	public function app_form(Request $request)
	{
		...
		if ($value != null) {
			...
			//$details = DB::Table('dcs.field_details')->where("formName", $request->app_form)->where('projectcode', '=', '60')->get();
	...
	public function store(Request $request)
	{
		...
		for ($i = 0; $i < $count1; $i++) {
			...
			$data->productname = $request->productname[$i];
			$data->groupname = $request->groupname[$i];
			$data->pagename = $request->pagename[$i];
			$data->pageno = $request->pageno[$i];
			$data->referencename = $request->referencename[$i];
			$data->table_name = $request->table_name[$i];
			$data->labelen = $request->labelEn[$i];
			$data->labelbn = $request->labelBn[$i];
